...
module.exports = async (data, Category, category) => {
  const errors = {};

  data.name = !isEmpty(data.name) ? data.name : '';
  data.plural_name = !isEmpty(data.plural_name) ? data.plural_name : '';

  if ((await Category.findOne({ name: data.name })) && category.name !== data.name) {
  ...
  if (!Validator.isEmpty(data.plural_name)) {
    if (
      (await Category.findOne({ slug: kebabCase(data.plural_name) })) &&
      category.slug !== kebabCase(data.plural_name)
    ) {
